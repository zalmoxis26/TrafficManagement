    ...
    private function procesarParArchivos(string $rutaTxtRel, string $rutaPdfRel): void
    {
        ...
        if (!$datos501) {
            ...
            $this->agregarEstatusSiFalta($fullTxtPath, $lineas, '*1001|Formato 501 inválido o incompleto');
              ...
              // NUEVO: también mandar el par a invoices/error
  ...
  private function enviarFacturaPorCorreo(Trafico $trafico): void
{
    try {
        ...
        // 1) Correos de la empresa (emailNotify)
        $emailsEmpresa = [];
        ...
        // 2) Usuarios con rol "documentador"
        $documentadores = User::role('documentador')
            ->where('id', '!=', $idExcluido)
            ->get(['id', 'name', 'email']);
        ...
        // Si no hay destinatarios válidos
        if (empty($to)) {
            return;
        }

        // 6) CC: documentadores + revisiones
