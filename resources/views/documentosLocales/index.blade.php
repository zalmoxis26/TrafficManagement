...
@section('content')
    ...
    <div class="container mt-5 content-box">
        ...
        <div class="directory-path">
            ...
            @if ($directory != '/')
                <a id="btnVolver" href="{{ route('documentsLocales.index', ['directory' => urlencode(dirname($directory))]) }}" class="btn btn-info mb-3 float-end text-white">Volver</a>
            @endif
        </div>
            ...
            <tbody>
                ...
                @foreach ($directories as $directoryItem)
                    <tr class="directory-row text-center">
                        <td class="bg-secondary-subtle">{{ basename($directoryItem['path']) }}</td>
                        <td class="bg-secondary-subtle">Carpeta</td>
                        <td class="text-center bg-secondary-subtle">
                            <a href="{{ route('documentsLocales.index', ['directory' => urlencode($directoryItem['path'])]) }}" class="btn btn-secondary">Abrir Carpeta</a>
                        </td>
                    </tr>
                @endforeach
                ...
                @foreach ($files as $file)
                    <tr class="text-center">
                        ...
                        <td class="text-center">
                            <a href="{{ route('documentsLocales.view', ['directory' => urlencode($directory), 'filename' => urlencode(basename($file['path']))]) }}" class="btn btn-primary">Ver</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
